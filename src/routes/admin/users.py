...
def admin_get_users(query: UserQuerySchema = Depends(), user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if query.username is not None:
        ...
        users_query = users_query.filter(or_(*[
            User.username.like(f"%{username}%") if username.startswith("*") and username.endswith("*") else \
            User.username.like(f"%{username}") if username.startswith("*") else \
            User.username.like(f"{username}%") if username.endswith("*") else \
    ...
    if query.sort_by is not None:
        if (query.sort_by == UserQuerySortByEnum.ID.value):
            users_query = users_query.order_by(desc(User.id) if query.desc is True else User.id)
        elif (query.sort_by == UserQuerySortByEnum.IS_ADMIN.value):
            users_query = users_query.order_by(desc(User.is_admin) if query.desc is True else User.is_admin)
        elif (query.sort_by == UserQuerySortByEnum.USERNAME.value):
            users_query = users_query.order_by(desc(User.username) if query.desc is True else User.username)
        elif (query.sort_by == UserQuerySortByEnum.EMAIL.value):
            users_query = users_query.order_by(desc(User.email) if query.desc is True else User.email)
        elif (query.sort_by == UserQuerySortByEnum.BIRTHDAY.value):
            users_query = users_query.order_by(desc(User.birthday) if query.desc is True else User.birthday)
        elif (query.sort_by == UserQuerySortByEnum.CREATED_AT.value):
            users_query = users_query.order_by(desc(User.created_at) if query.desc is True else User.created_at)
        else:
